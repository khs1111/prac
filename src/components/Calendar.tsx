...
import { CalendarIcon, Clock, Moon, Activity, Heart, Edit3, Save, Volume2, AlertTriangle, ChevronLeft, ChevronRight, Play, Video } from 'lucide-react';
...
export default function Calendar({ dayRecords, onUpdateDayMemo, onUpdateDayRecord }: CalendarProps) {
  const [selectedDate, setSelectedDate] = useState<Date | undefined>(new Date());
  const [memoText, setMemoText] = useState('');
  const [editMode, setEditMode] = useState(false);
  const [showVideoDialog, setShowVideoDialog] = useState(false);
  const [selectedVideoEvent, setSelectedVideoEvent] = useState<{type: string, time: string} | null>(null);
  ...
  const handleDateSelect = (date: Date | undefined) => {
    // 같은 날짜를 다시 클릭하면 선택 상태만 유지하고 다른 동작은 하지 않음
    if (date && selectedDate && date.toISOString().split('T')[0] === selectedDate.toISOString().split('T')[0]) {
      setSelectedDate(date); // 선택 상태 유지
      return;
    }
  ...
  return (
    <div className="h-full space-y-4">
        ...
        <div className="w-full">
          <CalendarComponent
            ...
            classNames={{
              ...
              cell: "flex-1 h-9 w-9 text-center text-sm p-0 relative focus-within:relative focus-within:z-20",
      ...
      <Card className="p-4 border-purple-100">
        <div className="flex items-center justify-between mb-4">
          <h3>
            {!editMode ? '수면 기록' : selectedDayRecord ? '기록 수정' : '수동 기록 추가'}
          </h3>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-purple-50 rounded-xl p-3 text-center border border-purple-100">
                ...
                <div className="text-lg text-purple-500">{formatTime(selectedDayRecord.sleepTime)}</div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setManualEntry({...manualEntry, sleepTime: e.target.value === '' ? 0 : Number(e.target.value)})}
                  placeholder="480"
                  className="mt-1"
                  min="0"
                />
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setManualEntry({...manualEntry, napTime: e.target.value === '' ? 0 : Number(e.target.value)})}
                  placeholder="90"
                  className="mt-1"
                  min="0"
                />
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setManualEntry({...manualEntry, tossingCount: e.target.value === '' ? 0 : Number(e.target.value)})}
                  placeholder="0"
                  className="mt-1"
                  min="0"
                />
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setManualEntry({...manualEntry, cryingCount: e.target.value === '' ? 0 : Number(e.target.value)})}
                  placeholder="0"
                  className="mt-1"
                  min="0"
                />
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setManualEntry({...manualEntry, fallCount: e.target.value === '' ? 0 : Number(e.target.value)})}
                  placeholder="0"
                  className="mt-1"
                  min="0"
                />
              ...
              <div>
                ...
                <Select 
                  ...
                  onValueChange={(value: 'excellent' | 'good' | 'fair' | 'poor') => 
                    setManualEntry({...manualEntry, sleepQuality: value})
                  }
                >
