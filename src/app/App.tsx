...
import { Home, CalendarIcon, FileText, Settings as SettingsIcon, Brain, AlertTriangle } from 'lucide-react'
...
export default function App() {
  ...
  const handleEventDetected = (eventType: string, severity: 'low' | 'medium' | 'high') => {
    ...
    setDayRecords(prev => {
      ...
      if (existing) {
        const updated = prev.map(r => {
          ...
          const next = { ...r, events: [...r.events, { type: eventType, time: hhmm, severity }] };
    ...
    if (severity === 'high') toast.error(`⚠️ ${eventType} 감지! 즉시 확인하세요.`);
    else if (severity === 'medium') toast.warning(`📢 ${eventType} 감지`);
    else toast.info(`ℹ️ ${eventType} 감지`);
  };
  ...
  const handleUpdateDayMemo = (date: string, memo: string) => {
    setDayRecords(prev => {
      ...
      if (existing) return prev.map(r => (r.date === date ? { ...r, memo } : r));
      return [...prev, { date, events: [], sleepTime: 0, napTime: 0, sleepQuality: 'good', tossingCount: 0, cryingCount: 0, fallCount: 0, memo }];
    });
    toast.success('메모가 저장되었습니다');
  };
  ...
  const handleUpdateDayRecord = (date: string, updated: Partial<DayRecord>) => {
    setDayRecords(prev => {
      const existing = prev.find(r => r.date === date);
      if (existing) return prev.map(r => (r.date === date ? { ...r, ...updated } : r));
      return [...prev, { date, events: [], sleepTime: 0, napTime: 0, sleepQuality: 'good', tossingCount: 0, cryingCount: 0, fallCount: 0, memo: '', ...updated }];
    });
    toast.success('수면 기록이 저장되었습니다');
  };

  const handleClearData = () => {
    setSleepRecords([]); setEvents([]); setDayRecords([]);
    toast.success('모든 데이터가 삭제되었습니다');
  };

  const generateAIInsights = () => {
    if (dayRecords.length === 0) return [];
    const avgToss = dayRecords.reduce((s, r) => s + r.tossingCount, 0) / dayRecords.length;
    const avgSleepHours = dayRecords.reduce((s, r) => s + (r.sleepTime + r.napTime), 0) / dayRecords.length / 60;
    const totalCry = dayRecords.reduce((s, r) => s + r.cryingCount, 0);
    const totalFall = dayRecords.reduce((s, r) => s + r.fallCount, 0);

    const insights: Array<{type:'warning'|'danger'|'good'|'info'; title:string; description:string}> = [];
    if (avgToss > 15) insights.push({ type: 'warning', title: '뒤척임 빈도 높음', description: `평균 ${Math.round(avgToss)}회의 뒤척임이 감지되었습니다. 수면 환경이나 침구류 점검을 권장합니다.` });
    else if (avgToss < 5) insights.push({ type: 'good', title: '안정적인 수면', description: '뒤척임이 적어 깊고 안정적인 수면을 취하고 있습니다.' });

    if (avgSleepHours < 6) insights.push({ type: 'warning', title: '수면 시간 부족', description: `평균 ${Math.round(avgSleepHours * 10) / 10}시간으로 권장 수면 시간보다 부족합니다.` });
    else if (avgSleepHours > 12) insights.push({ type: 'info', title: '충분한 수면', description: '충분한 수면 시간을 유지하고 있습니다.' });

    if (totalCry > 10) insights.push({ type: 'warning', title: '울음 빈도 증가', description: '울음이나 불편함을 나타내는 신호가 자주 감지되었습니다. 환경 점검이 필요할 수 있습니다.' });

    if (totalFall > 3) insights.push({ type: 'danger', title: '낙상 위험 감지', description: `${totalFall}건의 낙상 이벤트가 감지되었습니다. 침대 안전장치를 점검하세요.` });
    else if (totalFall === 0) insights.push({ type: 'good', title: '안전한 수면 환경', description: '낙상이나 위험한 상황이 감지되지 않았습니다.' });
  ...
  const aiInsights = dayRecords.length > 0 ? (
    <Card className="p-3 bg-gradient-to-br from-purple-50 to-indigo-50 border-purple-200">
      ...
      <div className="space-y-2">
        {generateAIInsights().map((insight, i) => (
          <div
            key={i}
  ...
  if (!selectedRole) return <RoleSelection onRoleSelect={handleRoleSelect} />

  if (selectedRole === 'camera' || showCameraMode) {
    return (
      <CameraMode                              
        onBack={() => { setSelectedRole(null); setShowCameraMode(false); }}
      />
    )
  }
        ...
        <div className="absolute bottom-0 left-0 w-full bg-white/95 backdrop-blur-sm border-t border-purple-100" style={{ width: '412px' }}>
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="grid w-full grid-cols-4 h-14 bg-transparent">
              <TabsTrigger value="home" className="flex flex-col items-center gap-1 data-[state=active]:bg-purple-50 data-[state=active]:text-purple-400">
                <Home className="w-5 h-5" /><span className="text-xs">홈</span>
              </TabsTrigger>
              <TabsTrigger value="calendar" className="flex flex-col items-center gap-1 data-[state=active]:bg-purple-50 data-[state=active]:text-purple-400">
                <CalendarIcon className="w-5 h-5" /><span className="text-xs">캘린더</span>
              </TabsTrigger>
              <TabsTrigger value="reports" className="flex flex-col items-center gap-1 data-[state=active]:bg-purple-50 data-[state=active]:text-purple-400">
                <FileText className="w-5 h-5" /><span className="text-xs">리포트</span>
              </TabsTrigger>
              <TabsTrigger value="settings" className="flex flex-col items-center gap-1 data-[state=active]:bg-purple-50 data-[state=active]:text-purple-400">
                <SettingsIcon className="w-5 h-5" /><span className="text-xs">설정</span>
              </TabsTrigger>
            </TabsList>
          </Tabs>
        </div>
